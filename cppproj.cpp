...
int rollno;
char name[50];
int eng_marks, math_marks, sci_marks, lang_marks, cs_marks, attendance;
double average;
char grade;
...
{
average=(eng_marks+math_marks+sci_marks+lang_marks+cs_marks)/5.0;
if(average>=90)
grade='A';
else if(average>=75)
grade='B';
...
cin.ignore();
cin.getline(name,50);
cout<<"\n\nEnter student's attendence till now: ";
cin>>attendance;
...
stud.getdata();
    int pos=(-1)*static_cast<int>(sizeof(stud));//to copy the same bit pattern to output
    fl.seekp(pos,ios::cur);// to set position in output
